...
class Emg_mod():
    def __init__(self, mnpool):
        ...
        self.length     = 300 # 肌肉长度 mm

        self.elect_z    = 14 # 阵列中沿肌纤维方向的电极数量
        self.elect_x    = 9 # 阵列中横跨肌纤维的电极数量
        self.elect_d    = 4 # 电极间距离 mm
        self.elect_loc  = 130 # 电极阵列位置 mm
        ...
        self.nmj_pos    = 150 # 神经肌肉接点位置（z轴） mm
        self.iz_width   = 8 # 神经支配区的宽度 5~20 mm
        # self.cv_mean    = 4.0 # 运动单元传导速度均值
        # self.cv_std     = 0.35 # 运动单元传导速度标准差
        # self.rc         = 0.02 # 运动单元的径向电导率S/m 西门子每米 
        # self.arc_ratio  = 6 # 轴向和径向电导率的比例 4~25
    ...
    def circle_tissues(self):
        ...
        circle = np.arange(0,2*np.pi,0.01) # 极坐标肌肉层边界点数组(角度)
        ...
        self.z = np.arange(0,self.length,1e-3) # z轴坐标
        
        self.elec = np.zeros((self.elect_z, self.elect_x, 3)) # 电极位置矩阵
        self.elec_size = [(self.elect_z-1) * self.elect_d, (self.elect_x-1) * self.elect_d] # 电极阵列的边长
        zs = np.linspace(self.elect_loc - self.elec_size[0]/2, self.elect_loc + self.elec_size[0]/2, self.elect_z) # z坐标
        self.elec[:,:,2] = np.tile(zs[:, np.newaxis], (1,self.elect_x)) # 每一行的z坐标相同
        angle = self.elec_size[1] / (self.r+self.fat+self.skin) # 根据电极阵列的x边长计算夹角
        angles = np.linspace(np.pi/2 - angle/2, np.pi/2 + angle/2, self.elect_x) # 每一列的夹角相同
        self.elec[:,:,0] = np.tile(((self.r+self.fat+self.skin) * np.cos(angles))[np.newaxis, :], (self.elect_z, 1)) # x坐标
        self.elec[:,:,1] = np.tile(((self.r+self.fat+self.skin) * np.sin(angles))[np.newaxis, :], (self.elect_z, 1)) # y坐标
    ...
    def view_morpho(self, csa, fat, skin, theta, prop, length, morpho, elect_z, elect_x, elect_d, elect_loc, save):
        """
        ...
        Args:
            ...
            fat (float): Adipose tickness [mm]. 脂肪层厚度
            skin (float): Skin tickness [mm]. 皮肤层厚度
            ...
            save (string): 
        """
        self.csa, self.skin, self.fat = csa*1e-6, skin*1e-3, fat*1e-3 # 转换为m
        self.theta, self.prop, self.morpho = theta, prop , morpho
        self.length = length*1e-3
        self.elect_z, self.elect_x, self.elect_d, self.elect_loc = elect_z, elect_x, elect_d*1e-3, elect_loc*1e-3
        self.save = save
        self.defineMorpho()
        ...
        ma_grid, z_grid = np.meshgrid(self.ma*1e3, self.z*1e2)
        mb_grid, z_grid = np.meshgrid(self.mb*1e3, self.z*1e2)
        fa_grid, z_grid = np.meshgrid(self.fa*1e3, self.z*1e2)
        fb_grid, z_grid = np.meshgrid(self.fb*1e3, self.z*1e2)
        sa_grid, z_grid = np.meshgrid(self.sa*1e3, self.z*1e2)
        sb_grid, z_grid = np.meshgrid(self.sb*1e3, self.z*1e2)
        
        ax.plot_surface(ma_grid, mb_grid, z_grid, alpha=0.3, color='blue')
        ax.plot_surface(fa_grid, fb_grid, z_grid, alpha=0.3, color='red')
        ax.plot_surface(sa_grid, sb_grid, z_grid, alpha=0.3, color='green')
        ax.scatter((self.elec[:,:,0]*1e3).flatten(), (self.elec[:,:,1]*1e3).flatten(), (self.elec[:,:,2]*1e2).flatten(), 
                   label='Electrode', color='green')
        ax.plot(self.ma*1e3, self.mb*1e3, ls='-.', color='blue')
        ax.plot(self.fa*1e3, self.fb*1e3, ls=':', color='red')
        ax.plot(self.sa*1e3, self.sb*1e3, color='green')
        ax.plot(self.ma*1e3, self.mb*1e3, np.full_like(self.mb, self.length*1e2), ls='-.', label='Muscle boundaries', color='blue')
        ax.plot(self.fa*1e3, self.fb*1e3, np.full_like(self.mb, self.length*1e2), ls=':', label='Fat boundaries', color='red')
        ax.plot(self.sa*1e3, self.sb*1e3, np.full_like(self.mb, self.length*1e2), label='Skin boundaries', color='green')
        ...
        ax.set_zlabel('cm')
        plt.tight_layout()
        plt.savefig(os.path.join(self.save, 'muscle-morphology_'+self.morpho+'_3D.tif'), dpi=600)
        
        plt.figure(figsize=(5,5))
        ax = plt.gca()
        rect1 = Rectangle((0, -self.r*1e3), self.length*1e2, 2*self.r*1e3, edgecolor='blue', facecolor='none', linewidth=2, ls='-.', 
                         label='Muscle boundaries')
        
        rect2 = Rectangle((0, -(self.r+self.fat)*1e3), self.length*1e2, 2*(self.r+self.fat)*1e3, edgecolor='red', facecolor='none', 
                         linewidth=2, ls=':', label='Fat boundaries')
        
        rect3 = Rectangle((0, -(self.r+self.fat+self.skin)*1e3), self.length*1e2, 2*(self.r+self.fat+self.skin)*1e3, edgecolor='green', 
                         facecolor='none', linewidth=2, label='Skin boundaries')
        ax.add_patch(rect1)
        ax.add_patch(rect2)
        ax.add_patch(rect3)
        plt.scatter((self.elec[:,:,2]*1e2).flatten(), (self.elec[:,:,0]*1e3).flatten(), marker='o', label='Electrode', color='green')
        plt.legend(loc='upper left')
        plt.axis('equal')
        plt.xlabel('cm')
        plt.ylabel('mm')
        plt.tight_layout()
        plt.savefig(os.path.join(self.save, 'muscle-morphology_'+self.morpho+'_2D_1.tif'), dpi=600)
        
        plt.figure(figsize=(5,5))
        plt.plot(self.ma*1e3, self.mb*1e3, ls='-.', label='Muscle boundaries', color='blue')
        plt.plot(self.fa*1e3, self.fb*1e3, ls=':', label='Fat boundaries', color='red')
        plt.plot(self.sa*1e3, self.sb*1e3, label='Skin boundaries', color='green')
        plt.scatter((self.elec[:,:,0]*1e3).flatten(), (self.elec[:,:,1]*1e3).flatten(), marker='o', label='Electrode', color='green')
        plt.legend(loc='upper left')
        plt.axis('equal')
        plt.xlabel('mm')
        plt.ylabel('mm')
        plt.tight_layout()
        plt.savefig(os.path.join(self.save, 'muscle-morphology_'+self.morpho+'_2D_0.tif'), dpi=600)
    ...
    def view_distribution(self, ratio, t1m, t1dp, t2m, t2dp):
        ...
        fill_blue = mpl.patches.Patch(label='Recruited type I MU', fc=(0,0,1,0.4))
        fill_red = mpl.patches.Patch(label='Recruited type II MU', fc=(1,0,0,0.4))
        blue_line = mpl.lines.Line2D([], [], color='b', label='Type I MU')
        red_line = mpl.lines.Line2D([], [], color='r', ls='--', label='Type II MU')
        plt.legend(handles=[fill_blue, fill_red, blue_line, red_line], loc='upper left', fontsize=8)
        ...
        plt.plot(self.ma*1e3, self.mb*1e3, ls='-.', color='blue')
        plt.plot(self.fa*1e3, self.fb*1e3, ls=':', color='red')
        plt.plot(self.sa*1e3, self.sb*1e3, color='green')
        plt.scatter((self.elec[:,:,0]*1e3).flatten(), (self.elec[:,:,1]*1e3).flatten(), marker='o', label='Electrode', color='green')
        plt.axis('equal')
        plt.sca(axes[1])
        axes1  = plt.gca()
        ...
        with plt.style.context('ggplot'):
            im1 = plt.imshow(hist.T, extent=extent, cmap=plt.cm.jet, interpolation='nearest', origin='lower')
    ...
    def exp_interpol(self):
        ...
        rte = self.mnpool.rte # 招募阈值兴奋
        self.amp_array = rte * (self.v2-self.v1)/self.rr +self.v1 # MUs中心的MUAP振幅
    ...
    def expn_interpol(self):  
        ...
        rte = 1/self.mnpool.rte # 招募阈值兴奋的倒数
        self.lm_array = rte*(self.d1-self.d2)+self.d2 # MUs中心的MUAP持续时间
        
        
    def motorUnitNMJs(self):
        """
        为每个运动单元随机分配神经肌肉接点的位置，即z坐标

        Args:
            nmj_pos (float): 神经肌肉接点的平均位置（mm）.
            iz_width (float): 神经支配区的宽度（mm）.
        """
        self.nmjs = np.ones(self.n) * self.nmj_pos
        for i in range(self.n):
            self.nmjs[i] = np.random.normal(self.nmj_pos, self.iz_width/2)
    ...
    def vc_filter(self):
        ...
        """
        self.mu_distance = np.zeros((self.elect_z, self.elect_x, self.n))
        for z in range(self.elect_z):
            for x in range(self.elect_x): # 计算每个运动单元与电极之间的距离
                self.mu_distance[z, x, :] = np.sqrt((self.elec[z,x,0]-self.x)**2+(self.elec[z,x,1]-self.y)**2+(self.elec[z,x,2]-self.nmjs)**2)
    ...
    def get_MUAPs(self):
        ...
        z = np.zeros((self.n, len(x))) # 振幅

        self.hr_order = np.zeros(self.n) # 每个MU的动作电位函数的阶数（随机分配）
        for i in range(self.n):
    ...
    def get_ptp(self):
        ...
        return x, muaps, ptp_amps, ptp_durs


    def view_muap(self, v1, v2, d1, d2):
        """
        ...
        x, z, amps, durs = self.get_ptp() # 计算MUs中心的MUAPs和PTP
        ...
        plt.xlabel('MU $i$')
        plt.ylabel('$\\lambda_M$ [ms]')
        plt.xlim(0,self.n)
        
        bins = 50
        ax[1,1].hist(amps,bins=bins)
        ax[1,1].set_xlabel('MUAP amplitude [mV]')
        ax[1,1].set_ylabel('Counts')
        
        ax[2,1].hist(durs,bins=bins)
        ax[2,1].set_xlabel('MUAP duration [ms]')
        ax[2,1].set_ylabel('Counts')

        plt.tight_layout()
        plt.savefig(os.path.join(self.save, 'MUAPs.tif'), dpi=600)
    ...
    def semg(self): 
        ...
        生成sEMG信号
        """
        self.raw_emg = np.zeros((self.elect_z, self.elect_x, len(self.t))) # 表面肌电信号
        self.mu_emg = np.zeros((self.elect_z, self.elect_x, self.LR, len(self.t))) # 每个激活MU对应的表面肌电信号
        # self.hr_order = np.zeros(self.LR) # 每个激活MU的动作电位函数的阶数（随机分配）
        for i in range(self.LR):
            for z in range(self.elect_z):
                for x in range(self.elect_x):
                    if self.hr_order[i] == 2: 
                        temp = self.hr2_f(self.t,self.ampvar[z,x,i],self.lmvar[z,x,i],self.neural_input[i])
                    else:
                        temp = self.hr1_f(self.t,self.ampvar[z,x,i],self.lmvar[z,x,i],self.neural_input[i])
                    self.mu_emg[z, x, i] = temp
                    self.raw_emg[z, x] = self.raw_emg[z, x] + temp
        
        
    def get_semg(self, nmj_pos, iz_width, ampk, durak, add_noise, noise_level, add_filter, order, bplc, bphc):
        """
        获取添加噪声和滤波之后的表面肌电信号        
        ...
        """
        self.nmj_pos, self.iz_width = nmj_pos*1e-3, iz_width*1e-3
        self.ampk, self.durak = ampk*1e-3, durak*1e3  
        ...
        if self.neural_input == []:
            ...
            print("Processing...")
            self.motorUnitNMJs() # 计算MUs的神经肌肉接点位置
            self.vc_filter() # 计算表面电极记录到的MUAPs振幅和持续时间
            self.semg() # 计算肌电信号
            if add_noise:
                for z in range(self.elect_z):
                    for x in range(self.elect_x):
                        self.emg[z, x] = self.raw_emg[z, x]  + np.random.normal(0, noise_level, len(self.raw_emg[z,x]))
            else:
                self.emg = self.raw_emg
            if add_filter:
                self.butter_bandpass_filter(order)


    def view_semg(self, nmj_pos, iz_width, ampk, durak, add_noise, noise_level, add_filter, order, bplc, bphc):
        """
        ...
        self.get_semg(nmj_pos, iz_width, ampk, durak, add_noise, noise_level, add_filter, order, bplc, bphc)
        emg = self.emg
        emg = (emg-emg.mean()) / (emg.max()-emg.min())
        emg = emg.reshape(self.elect_z*self.elect_x, len(self.t))
        
        fig = plt.figure(figsize = (self.elect_z, self.elect_x))
        for i in np.arange(1, self.elect_z * self.elect_x + 1, 1):
            ax = plt.subplot(self.elect_z , self.elect_x, i)
            plt.subplots_adjust(wspace = 0.0, hspace = 0.0)
            ax.grid(which = 'both', ls = 'dashed')
            plt.plot(self.t, emg[i-1, :])
            plt.xlim(self.t[0], self.t[-1]+1)
            if (i-1) < (self.elect_z*self.elect_x - self.elect_x):
                ax.xaxis.set_major_formatter(NullFormatter())
            plt.ylim=(emg.min()-1, emg.max()+1)
            ax.yaxis.set_major_formatter(NullFormatter())
            if (i-1) == 0:
                ax.set_ylabel(1, rotation = 0, ha = 'center', va = 'center', fontsize = 14)
            for j in range(i-1):
                if (i-1) == np.append(np.arange(0, self.elect_z*self.elect_x, self.elect_x), np.zeros(self.elect_z*self.elect_x))[j]:
                    ax.set_ylabel(np.arange(1, self.elect_z * self.elect_x + 1, 1)[j], rotation = 0, ha = 'center', va = 'center', fontsize = 14)
                elif self.elect_x == 1:
                    ax.set_ylabel(np.arange(1, self.elect_z * self.elect_x + 1, 1)[j]+1, rotation = 0, ha = 'center', va = 'center', fontsize = 14)
        ...
        if self.elect_x > 1:
            ...
            fig.supxlabel('Time (s)')
        if self.elect_z > 1:
            fig.supylabel('Normalized sEMG signal\n Electrodes in the z direction, i.e. along the fibre', ha = 'center', va = 'center')
        else: 
            fig.supylabel('Normalized sEMG signal', ha = 'center', va = 'center')
    ...
    def butter_bandpass_filter(self, order=4):
        ...
        for z in range(self.elect_z):
            for x in range(self.elect_x):
                self.emg[z,x,:] = filtfilt(b, a, self.emg[z,x,:])
